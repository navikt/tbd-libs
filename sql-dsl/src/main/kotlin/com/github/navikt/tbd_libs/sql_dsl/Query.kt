...
fun <R> DataSource.connection(block: Connection.() -> R): R = connection.use(block)

// krever minst én rad og at mapping-funksjonen ikke returnerer null
fun <R> ResultSet.single(map: (ResultSet) -> R?): R = checkNotNull(singleOrNull(map)) { "forventet ikke er null-verdi" }

// krever én rad, men mapping-funksjonen kan returnere null
fun <R> ResultSet.singleOrNull(map: (ResultSet) -> R?): R? = this.map(map).single()

// returnerer null hvis resultatet er tomt eller mapping-funksjonen returnerer null
fun <R> ResultSet.firstOrNull(map: (ResultSet) -> R?): R? = this.map(map).firstOrNull()
...
private fun <R> PreparedStatement.execute(map: ResultSet.() -> R) = use { map(it.executeQuery()) }

@OptIn(ExperimentalContracts::class)
private fun <T : Any> checkNotNull(
...
private fun <T> ResultSet.nullable(
    columnName: String,
    notNull: () -> T,
): T? {
...
fun Connection.prepareStatementWithNamedParameters(
    sql: String,
...
private fun buildPreparedStatement(
    ...
    orderOfNamedParameters: List<PreparedStatement.(Int) -> Unit>,
): PreparedStatement {
...
internal fun extractNamedParametersFromQuery(sql: String): Pair<String, List<String>> {
    ...
    val orderOfNamedParameters: List<String> =
        namedParameterRegex
            .findAll(sql)
...
data class ParametersBuilder(
    ...
    inline fun <reified T> withParameter(
        name: String,
        value: List<T>,
    ) {
        val sqlType =
    ...
    @LocalDateTimeIDatabase(
        message =
            "LocalDateTime representerer ikke et forenlig punkt i tid og bør unngås til bruk i databaser. " +
    ...
    fun withParameter(
        name: String,
        valueSetter: PreparedStatement.(column: Int) -> Unit,
    ) = apply {
...
annotation class LocalDateTimeIDatabase(
    val message: String,
)
