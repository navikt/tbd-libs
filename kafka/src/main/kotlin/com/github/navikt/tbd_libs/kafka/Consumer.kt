...
fun AdminClient.getPartitions(topic: String) =
    this
 ...
 */
fun AdminClient.findOffsetsForTime(
    topic: String,
    time: LocalDateTime,
...
fun <K, V> KafkaConsumer<K, V>.poll(
    until: () -> Boolean = { true },
    block: (ConsumerRecords<K, V>) -> Unit,
) {
    while (until()) {
