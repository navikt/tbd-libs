...
class SpedisjonClient(
    ...
    fun hentMeldinger(
        internDokumentIder: List<UUID>,
        callId: String = UUID.randomUUID().toString(),
    ): Result<HentMeldingerResponse> =
        tokenProvider
            .bearerToken(scope)
            .map { token ->
    ...
    private fun <T> håndterRespons(
        response: HttpResponse<String>,
        mapRespons: (status: Int, body: String) -> Result<T>,
    ): Result<T> {
