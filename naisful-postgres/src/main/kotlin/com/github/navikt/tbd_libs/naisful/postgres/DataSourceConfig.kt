 ...
 */
fun defaultJdbcUrl(
    metode: ConnectionConfigFactory = ConnectionConfigFactory.Env(),
    options: Map<String, String> = emptyMap(),
): String? = metode.buildJdbcUrl(options)
 ...
 * @see <a href="https://doc.nais.io/persistence/postgres/reference/?h=jdbc#database-connnection">nais doc</a>
 */
fun jdbcUrlWithGoogleSocketFactory(
    databaseInstance: String,
    metode: ConnectionConfigFactory,
    gcpTeamProjectId: String = System.getenv("GCP_TEAM_PROJECT_ID"),
    databaseRegion: String = "europe-north1",
): String? =
    defaultJdbcUrl(
        metode,
        mapOf(
...
sealed class ConnectionConfigFactory {
    ...
    data class Env(
        ...
        override fun buildJdbcUrl(options: Map<String, String>): String? =
            buildJdbcUrl(options) { key ->
                env.getKeySuffixOrNull(envVarPrefix, key)
            }
    }

    data class MountPath(
        val path: String,
    ) : ConnectionConfigFactory() {
        override fun buildJdbcUrl(options: Map<String, String>): String? {
...
fun Map<String, String>.getKeySuffixOrNull(
    prefix: String?,
    suffix: String,
): String? {
...
private fun buildPostgresCompliantJdbcUrl(
    hostname: String,
    port: Int,
    databaseName: String,
    username: String,
...
private fun optionsString(options: Map<String, String>): String = (options).entries.joinToString(separator = "&") { (k, v) -> "$k=$v" }
