...
class AzureTokenClientTest {
    ...
    fun `fetch token from client secret`() {
        ...
        result as AzureTokenProvider.AzureTokenResult.Ok
        assertEquals("eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsIng1dCI6Ik1uQ19WWmNBVGZNNXBP...", result.azureToken.token)
    ...
    private fun requestToken(authMethod: AzureAuthMethod): Pair<HttpClient, AzureTokenProvider.AzureTokenResult> {
    ...
    private fun requestOboToken(authMethod: AzureAuthMethod): Pair<HttpClient, AzureTokenProvider.AzureTokenResult> {
        val httpClient = mockOkResponse()
        val client = AzureTokenClient(tokenEndpoint, CLIENT_ID, authMethod, httpClient)
